...
public:
	...
	double abandonArea;          //不良区域或褶皱的像素个数
	...
	void SetParameter(double m_resolution, CString m_savePath, vector<CString> vecPicPaths);   //设置参数（从主类中传进参数）
	void Initialize();   //参数初始化
	void ComputeAccuracy(Mat srcImg);     // 保存结果图并精确计算出各项参数
	void Thresh(Mat &srcImag, Mat &dstImag);   //二值化图像
